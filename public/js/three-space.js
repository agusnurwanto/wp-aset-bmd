...
class Thpace {
    ...
    animate() {
        ...
        this.triangles.forEach(t => {
            ...
            var dp = [0, 1, 2, 0];
            dp.forEach((el, ind) => {
                if (this.coordinateTable[coords[el].x] && this.coordinateTable[coords[el].x]
                    [coords[el].y] != undefined) {
    ...
    generateTriangles() {
        ...
        for (let i = 0; i < triangleList.length; i += 3) {
            ...
            var color = gradient(getCenter(coords), this.width, this.height, this.settings.color1, this
                .settings.color2);
    ...
    animateCoordinateTable() {
        Object.keys(this.coordinateTable).forEach(x => {
            Object.keys(this.coordinateTable[x]).forEach(y => {
                ...
                var changeX = Math.cos(this.baseCoordinateTable[x][y]) * this.settings
                    .pointVariationX;
                var changeY = Math.sin(this.baseCoordinateTable[x][y]) * this.settings
                    .pointVariationY;
...
function gradient(coords, width, height, leftColor, rightColor) {
    ...
    var r = Math.ceil(parseInt(color1.substring(1, 3), 16) * per + parseInt(color2.substring(1, 3), 16) * (1 -
        per));
    var g = Math.ceil(parseInt(color1.substring(3, 5), 16) * per + parseInt(color2.substring(3, 5), 16) * (1 -
        per));
    var b = Math.ceil(parseInt(color1.substring(5, 7), 16) * per + parseInt(color2.substring(5, 7), 16) * (1 -
        per));
...
let canvas = document.getElementById('canvas-download');
